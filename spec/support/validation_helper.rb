module ValidationHelper
  ...
  def validates_email_format_of(column_name, valid_value, invalid_value)
    subject.send(:"#{column_name}=", invalid_value)
    subject.valid?.should be_false
    subject.errors[column_name].should_not be_empty
    subject.send(:"#{column_name}=", valid_value)
  ...
  def validates_inclusion_of(column_name, bad_value, values)
    subject.valid?.should be_false
    subject.errors[column_name].should_not be_empty
    subject.send(:"#{column_name}=", bad_value)
    subject.valid?
    subject.errors[column_name].should_not be_empty
    values.each do |v|
      subject.send(:"#{column_name}=", v)
      subject.valid?
      subject.errors[column_name].should be_empty
    end
  end
